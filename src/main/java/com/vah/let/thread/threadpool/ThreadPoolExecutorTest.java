package com.vah.let.thread.threadpool;
...
//		 cachPool();
    ...
    private static void schedulePool() {
        ...
        ScheduledFuture<?> scheduledFuture = scheduledThreadPool.scheduleAtFixedRate(() -> System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "   延迟一秒执行,每三秒执行一次  " + Thread.currentThread().getName()), 1, 100, TimeUnit.MILLISECONDS);
    }
    ...
    private static void testThreadPoolExecutor() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            poolExecutor.execute(() -> {
                System.out.println(Thread.currentThread().getName() + " work start" + " " + finalI);
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                System.out.println(Thread.currentThread().getName() + " work end" + " " + finalI);
            });
        }
