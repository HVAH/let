 ...
 * right 子树在中序中的右边界
 */
fun build(preorder: IntArray, root: Int, left: Int, right: Int): TreeNode? {
    if (left > right) {
        return null
    }
    val p = preorder[root]
    ...
    node.left = build(preorder, root + 1, left, pIndex - 1)
    ...
    // 右子树的右边界就是当前的又边界
    node.right = build(preorder, root + (pIndex - left) + 1, pIndex + 1, right)
    return node
}
